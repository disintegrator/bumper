...
When `bumper latest --group my-package` is called with this built-in command as the `current_cmd`, it will output `1.2.3`.
...
Then running `bumper latest --group my-package` will output `1.2.3`. The file path is relative to the workspace root (the directory that holds the `.bumper` directory).
